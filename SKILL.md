...
6. **Customer Updates** — collapsed by default, one collapsible card per assigned account (Section 3). Each card has an editable textarea pre-populated with the generated update, a channel field, a "Post to Slack" button, and the last-known-update timestamp.
7. **Manager / Leadership Update** — collapsed by default, single editable textarea (Section 4), a "Post to Manager" button, and the last-known-update timestamp.
