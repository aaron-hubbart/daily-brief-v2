...
- **`content`** — `{}` for `checkable`/`fyi` items in most sections (everything needed to render lives in title/subtitle/badge/links), **except Action Items**, which requires `{"due_on": "YYYY-MM-DD" or null, "is_new": true|false, "project_name": "..." or null}` — see the Action Items note below for what populates these. For `card`: `{"textarea": "...", "channel_id": "...", "last_posted_at": "..."}`. For `text-block`: `{"textarea": "..."}`.
...
Populate `content.project_name` from the task's own project membership, not from the account mapping used to create it — a task found via search (rather than newly created) may already sit in a different project than what the account config would suggest, and this field should reflect where the task actually lives. Do not assume `search_tasks`/`create_tasks` responses already include project names — they don't reliably return the `projects` field without it being explicitly requested, which is why this previously defaulted to null on every item. For every action item, after it's created or matched, call `Asana:get_task` on its GID with `opt_fields: "projects.name"` and read the project name(s) from that response — this is a required step, not an optional enrichment. Use the first project's `name`; if the task belongs to more than one project, join the names with ", ". Only set `project_name` to `null` when that `get_task` call genuinely returns an empty `projects` array (the task really has no project, e.g. it landed in My Tasks because the account had no configured GID) — never as a fallback for a call you skipped or that returned an unexpected shape. For items where meeting-manager applies (post-meeting processing needed, most commonly a missing recording/transcript from Yesterday's Meetings), add a second `links` entry using the same `claude://` deep-link pattern: `href="claude://claude.ai/new?q=" + encodeURIComponent('/meeting-manager Run post-meeting notes for: ' + meetingTitle + ' (' + dateOrTime + ')')`. Only add this when meeting-manager genuinely applies. Note once in the brief output (not per item) that this link requires Claude Desktop registered as the `claude://` protocol handler.
...
- **`content.project_name`** — the Asana board (project) the task belongs to, per the note above. Rendered by the webapp as a small label on the item so it's obvious at a glance whether an action item is a TAM Recurring Activity, tied to a specific account's project, or sitting in My Tasks. Not used for subsection placement, unlike `due_on`/`is_new` — it's display-only.

Get all three fields right on every upsert; the webapp does the due-date/overdue-window math and subsection placement, not this skill, but it can only place an item correctly if `due_on` and `is_new` are accurate, and can only show the board label if `project_name` is present. Once created, the due date on an Asana-backed action item can also be edited directly from the webapp's due-date box (see `viewer/webapp/templates/brief_fragment.html`) — that write goes straight to Postgres and the linked Asana task, bypassing this skill entirely, so a subsequent brief run's upsert should reflect whatever `due_on` Asana now reports rather than assuming its own last-known value is still current.
