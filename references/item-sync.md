...
2. **Updating existing files** (`config.json` and the status-update cache
   `status-cache.json`, both in `/config/`): use the **title-based lookup
   + create-new/trash-old** pattern described in "Google Drive write
   mechanics" in `SKILL.md`. The connector's `update_file` only changes
   metadata, not content, so an "update" is: find the file by title in
   `/config/`, create a new file with the same title and parent, then
   trash the old one.
...
- **`item_type`** — one of `checkable`, `card`, `text-block`. Sections 1 (both parts), Today, Action Items, and FYI are all `checkable` — FYI items render with the same checkbox the other sections use, so the person can mark one off once they've read it — except Today's single `today-standup` entry, which is `card`. Customer Updates cards are `card`. Manager Update is `text-block`.
...
**Every Today meeting item also always carries two `claude://` deep-links, regardless of meeting type, size, or whether prep already exists — no qualifying criteria, no exceptions:** (this rule is about meeting items specifically; the section's one non-meeting entry, the `today-standup` card described above, carries no `links` at all)
