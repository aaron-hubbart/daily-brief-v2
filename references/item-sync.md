...
Populate `content.project_name` from the task's own project membership, not from the account mapping used to create it — a task found via search (rather than newly created) may already sit in a different project than what the account config would suggest, and this field should reflect where the task actually lives. Read it off the `projects` array on the Asana task object (`get_task`/`search_tasks`/`create_tasks` all return this): use the first project's `name`. If the task belongs to more than one project, join the names with ", ". If the task has no project (landed in My Tasks because the account had no configured GID), set `project_name` to `null` — the webapp treats `null` as "My Tasks", not a missing value. For items where meeting-manager applies (post-meeting processing needed, most commonly a missing recording/transcript from Yesterday's Meetings), add a second `links` entry using the same `claude://` deep-link pattern: `href="claude://claude.ai/new?q=" + encodeURIComponent('/meeting-manager Run post-meeting notes for: ' + meetingTitle + ' (' + dateOrTime + ')')`. Only add this when meeting-manager genuinely applies. Note once in the brief output (not per item) that this link requires Claude Desktop registered as the `claude://` protocol handler.
