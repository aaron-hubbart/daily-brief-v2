...
New Items' due date can still be edited directly from the webapp's due-date box after the fact (see `viewer/webapp/templates/brief_fragment.html`) — that write goes straight to that date's own `/state/{date}.json` file (via `drive_store.py`'s `set_item_due_date`) and the linked Asana task, bypassing this skill entirely, so if the same task is still new enough to be re-written on a same-day patch, reflect whatever `due_on` Asana now reports rather than assuming this skill's own last-sent value is still current.
