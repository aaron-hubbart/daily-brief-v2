...
3. **Download and parse.** Pull the file content. Find `<section data-section="customer-updates">` and within it the `<details data-id="cust-update-{slug}">` node for the target account (or `<section data-section="manager-update">` / `<details data-id="mgr-update">` for the manager target). Compute `{slug}` from the account name using the same lowercase-hyphenate rule as generation. If the node isn't found, stop and tell the user the card wasn't in the latest file rather than fabricating one.
