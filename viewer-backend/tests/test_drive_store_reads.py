...
def test_get_items_for_day_flattens_sections_and_applies_state_overrides(mocker):
    ...
    def fake_download(tok, file_id):
        ...
        return {
            'id-2026-07-21': None,
            'id-meetings.json': [
                {'item_key': 'ym-0900-acmefin', 'item_type': 'checkable', 'title': 'AcmeFin Sync',
            ...
            'id-2026-07-21.json': {
                'yesterday-meetings:ym-0900-acmefin': {'checked': True, 'due_on_override': None},
    ...
    assert by_key['yesterday-meetings:ym-0900-acmefin']['checked'] is True
...
def test_get_account_projects_reads_config_file(mocker):
    ...
    mocker.patch('drive_store._download_json', return_value={
        'accounts': [
            {'account_name': 'Acme Financial', 'project_gid': '111', 'slack_channel_id': 'C0XXXXXXX'},
        ]
    })

    result = drive_store.get_account_projects('tok', 'root-folder')
    assert result == [{'account_name': 'Acme Financial', 'project_gid': '111'}]
